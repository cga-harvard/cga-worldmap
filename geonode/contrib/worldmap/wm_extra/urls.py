...
from .views import (ajax_snapshot_history, layer_searchable_fields,
                    ...
                    map_detail_wm, map_view_wm_mobile, add_endpoint, printmap, layer_detail_wm )
...
urlpatterns = [
    ...
    url(r'^maps/(?P<mapid>\d+)/data$', map_json_wm, name='map_json'),
    url(r'^maps/(?P<mapid>\d+)$', map_detail_wm, name='map_detail_wm'),
    url(r'^maps/(?P<mapid>\d+)/view$', map_view_wm, name='map_view_wm'),
    url(r'^maps/(?P<mapid>\d+)/edit$', map_view_wm, name='map_view_wm'),
    url(r'^maps/(?P<mapid>\d+)/mobile$', map_view_wm_mobile, name='map_view_wm_mobile'),
    url(r'^maps/add_endpoint?$', add_endpoint, name='add_endpoint'),
    url(r'^snapshot/create/?$', snapshot_create, name='snapshot_create'),
    url(r'^maps/(?P<mapid>\d+)/(?P<snapshot>[A-Za-z0-9_\-]+)/$', map_view_wm, name='map_view_wm'),
    # wm history
    url(r'^maps/history/(?P<mapid>\d+)/?$', ajax_snapshot_history, name='ajax_snapshot_history'),
    # layers
    url(
            r'^data/(?P<layername>geonode:[^/]*)$',
            RedirectView.as_view(pattern_name='layer_detail', permanent=False)
       ),
    url(
        r'^layers/(?P<layername>geonode:[^/]*)/searchable-fields$',
        layer_searchable_fields,
        name='layer_searchable_fields'
       ),
    url(
        r'^data/(?P<layername>geonode:[^/]*)/ajax-edit-check/?$',
        ajax_layer_edit_check,
        name='ajax_layer_edit_check'),
    url(
        r'^data/(?P<layername>geonode:[^/]*)/ajax-edit-style-check/?$',
        ajax_layer_edit_style_check,
        name='ajax_layer_edit_style_check'),
    url(
        r'^data/(?P<layername>geonode:[^/]*)/ajax_layer_update/?$',
        ajax_layer_update,
        name='ajax_layer_update'),
    url(r'^layers/(?P<layername>geonode:[^/]*)$', layer_detail_wm, name="layer_detail_wm"),
